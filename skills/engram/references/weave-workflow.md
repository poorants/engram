...
the user wants it "more connected / more neural", or `engram lint` reports a low
`woven_ratio` and many `weak_nodes`.

## Steps

1. **Measure** — run `engram lint --json` and read `metrics` (`woven_ratio`,
...
2. **Find candidates** — run the muscle:
   ...
   engram weave --json
   ```
   It returns two advisory lists (never auto-applied):
...
4. **Re-measure & report** — re-run `engram lint --json`; report `woven_ratio`
   and `weak_nodes` before→after, plus the links woven and concept notes created.

## Caveat — respect the brain boundary

`engram weave` can surface clusters that are really **external deliverables**
